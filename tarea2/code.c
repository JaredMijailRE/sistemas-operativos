...
// includes para E/S y manejo de procesos

typedef int bool;

// emulación sencilla de booleanos en C
...
// tamaño actual usado para reasignaciones dinámicas
...
{
    int i;
    // imprime información básica y comandos integrados
...
int lsh_launch(char **args)
{
    ...
    if(pid == 0){ /* child process */
        // en el hijo: reemplazar la imagen del proceso con exec
        if(execvp(args[0], args) == -1) perror("lsh");
        exit(EXIT_FAILURE);
    }else if(pid > 0){ /* parent process */
        // en el padre: esperar al hijo a menos que se pida concurrencia
        if(!conc){
            do{
...
char **lsh_split_line(char *line){
    ...
    // dividir la línea en tokens usando los delimitadores definidos (strtok)
    ...
    // ver si ultima token es & para correr en segundo plano
...
int lsh_history(char **args)
{
    ...
    if(strcmp(args[0], "history") == 0){
        // mostrar historial desde el más reciente hasta el más antiguo
        ...
        char *command;
        if(strcmp(args[0], "!!") == 0){
            // ejecutar el último comando almacenado
            ...
            // calcular la posición en el historial según el número dado
            int offset = args[0][1] - '0'; // (puede ser cualquier ascii)
...
int lsh_execute(char *line){
    ...
    if(args[0] == NULL){ /* empty command was entered */
        // entrada vacía: no ejecutar nada
        return 1;
    ...
    while(*temp_args != NULL){
        // concatenar tokens para formar la cadena a guardar en historial (usa strcat)
...
char *lsh_read_line(void)
{
    ...
    // leer carácter a carácter hasta newline o EOF
    while(1){
        /* Read a character */
        c = getchar();
